  ...
  if (this.fields.length >= fieldIndex) {
    ...
    if (components.length >= componentIndex) {
      ...
      if (subComponentIndex) {
        var subComponent = Array.isArray(component.value[0]) ? component.value[0] : [component.value[0]];
        if (subComponent.length >= subComponentIndex) {
          return subComponent[subComponentIndex - 1].toString(delimiters)
        }
        return "";
      }
